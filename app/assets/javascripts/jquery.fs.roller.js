	...
	function _build($roller, opts) {
		if (!$roller.data("roller")) {
			...
			if (opts.controls && !$roller.find(".roller-controls").length) {
				...
				html += '<span class="roller-control previous glyphicon glyphicon-chevron-left">' + opts.labels.previous + '</span>';
				html += '<span class="roller-control next glyphicon glyphicon-chevron-right">' + opts.labels.next + '</span>';
				html += '</div>';
			}
